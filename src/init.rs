...
pub fn init() -> Result<()> {
    let rustlings_dir = Path::new("interview");
        ...
        .success()
    {
        bail!(
            "Clippy, the official Rust linter, is missing.\n\
             Please install it first before initializing."
        )
    }
    ...
    if locate_project_output.status.success() {
        ...
        stdout.write_all(b"This command will create the directory `interview/` as a member of this Cargo workspace.\n\
        ...
        stdout.write_all(b"The directory `interview` has been added to `workspace.members` in the `Cargo.toml` file of this Cargo workspace.\n")?;
        fs::remove_dir_all("interview")
            .context("Failed to remove the temporary directory `interview/`")?;
        init_git = false;
    } else {
        stdout.write_all(b"This command will create the directory `interview/` which will contain the exercises.\n\
                           Press ENTER to continue ")?;
        press_enter_prompt(&mut stdout)?;
    }

    create_dir(rustlings_dir).context("Failed to create the `interview/` directory")?;
    set_current_dir(rustlings_dir)
        .context("Failed to change the current directory to `interview/`")?;

    let info_file = InfoFile::parse()?;
    EMBEDDED_FILES
        .init_exercises_dir(&info_file.exercises)
        .context("Failed to initialize the `interview/exercises` directory")?;
    ...
    .context("Failed to create the file interview/solutions/README.md")?;
    ...
    fs::write("Cargo.toml", updated_cargo_toml)
        .context("Failed to create the file `interview/Cargo.toml`")?;

    fs::write("rust-analyzer.toml", RUST_ANALYZER_TOML)
        .context("Failed to create the file `interview/rust-analyzer.toml`")?;

    fs::write(".gitignore", GITIGNORE)
        .context("Failed to create the file `interview/.gitignore`")?;

    create_dir(".vscode").context("Failed to create the directory `interview/.vscode`")?;
    fs::write(".vscode/extensions.json", VS_CODE_EXTENSIONS_JSON)
        .context("Failed to create the file `interview/.vscode/extensions.json`")?;
...
You probably already initialized the interview exercises.
Run `cd interview`
Then run `viasat-interview` again";

const POST_INIT_MSG: &[u8] = b"Run `cd interview` to go into the generated directory.
Then run `viasat-interview` to get started.
";
